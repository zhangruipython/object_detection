...
class PictureMix:
    ...
    def photo_mix(self, img_part_list):
        ...
        if img_part_list[0].shape[1] == self.original_img_width:
            ...
            # # 左侧图像去除黑色区域，并在左侧图像左侧添加黑色区域用于保持两张图像的分辨率一致
            # trim_part01, trim_part02 = self.trim(img_part01), self.trim(img_part02)
            # # 在右侧图像的右边缘进行黑色填充
            # if trim_part01.shape[1] > trim_part02.shape[1]:
            #     trim_part01 = trim_part01[0:trim_part01.shape[0], 0:trim_part02.shape[1]]
            #     # add_part_w = trim_part01.shape[1] - trim_part02.shape[1]
            #     # trim_part02 = cv2.copyMakeBorder(trim_part02, 0, 0, 0, add_part_w, cv2.BORDER_CONSTANT, value=[0, 0, 0]
            # elif trim_part01.shape[1] < trim_part02.shape[1]:
            #     add_part_w = img_part02.shape[1] - trim_part01.shape[1]
            #     # 黑色填充
            #     trim_part01 = cv2.copyMakeBorder(trim_part01, 0, 0, add_part_w, 0, cv2.BORDER_CONSTANT, value=[0, 0, 0])
        ...
        # 图像拼接后在右侧会有小部分黑色区域
        return stitch_img[0:stitch_img.shape[0], 0:int(stitch_img.shape[1] * 0.98)]


if __name__ == '__main__':
    img01_path = "C:\\rongze\\data\\yike_picture\\demo06\\100.jpg"
    img02_path = "C:\\rongze\\data\\yike_picture\\demo06\\150.jpg"
    joint_img_path = "C:\\rongze\\data\\yike_picture\\demo06\\mix100_150.jpg"
